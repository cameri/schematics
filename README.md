...
| [`book-fetching`](schematics/book-fetching/SCHEMATIC.md) | media | A spec-only schematic: gluetun as a structural VPN kill switch, Deluge sharing its netns, Chaptarr automating grabs, and IP-sync for single-IP indexers; tailnet-only UIs |
| [`book-serving`](schematics/book-serving/SCHEMATIC.md) | media | A spec-only schematic: Audiobookshelf reading the same dataset the fetcher writes, exposure as a first-class parameter (Tailscale by default, tsdproxy or Cloudflare for a hostname) |
| [`book-stack`](schematics/book-stack/SCHEMATIC.md) | media | A composition schematic: no images of its own; it wires book-fetching and book-serving together through a shared volume contract and one grab-to-listen acceptance test |
