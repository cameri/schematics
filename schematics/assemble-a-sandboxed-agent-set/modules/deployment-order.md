...
| The alias set read back from the router differs from the decided list | Stop at Phase 3 and fix the router first: a layer built against a different set fails its own alias check after a pointless build |
...
| the router is running and serving `P-11 ROUTER_ALIAS_SET` → the harness layer build (a build-time check inside the layer part; Phase 4 step 3) | The layer validates `P-10 ROUTER_ALIAS` against the running router while it is built, so the alias is not a run-time surprise | A layer whose configured alias no provider on this router serves — found only after the host, the tree and the proxy are up | `curl -fsS "${ROUTER_BASE_URL%/v1}/health/liveliness"` answers, then `curl -fsS -H "Authorization: Bearer $ROUTER_API_KEY" "$ROUTER_BASE_URL/models" \| jq -e --arg a "$ROUTER_ALIAS" '.. \| objects \| select(.id?) \| .id \| select(. == $a)'` succeeds (the header carries the variable `P-9` names; its default is `ROUTER_API_KEY`). `A-4` asserts the edge by building with the router stopped |
...
| Build the harness layer with the router stopped, or with an alias it does not serve | The layer part's own alias check fails the build, naming the alias and the set it queried (`A-4`) | The check runs inside the build, before `HARNESS_IMAGE` exists. Found at run time instead, the mistake surfaces as an agent pane whose model calls fail with everything else already deployed |
