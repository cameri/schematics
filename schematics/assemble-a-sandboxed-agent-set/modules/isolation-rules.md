...
| No host root: no host namespace, no bind of `/`, and not uid 0 **in the container the agents run in** | The spine's `R-4`, whose prohibition is *host* root; part 1's `R-1` (a fixed, unprivileged account) | `docker exec "$HOST_CONTAINER" id -u` prints `1000`, not `0`; `docker inspect "$C" --format '{{.Config.User}} {{.HostConfig.PidMode}} {{.HostConfig.NetworkMode}} {{.HostConfig.IpcMode}}'` names no `host`; `docker inspect "$C" --format '{{range .Mounts}}{{.Source}}{{"\n"}}{{end}}' \| grep -xF /` prints nothing. A uid 0 **inside** another container of the set that holds no host namespace, no socket and no bind of `/` is container root, not host root: `A-7` prints it in a note and does not fail it — the two parts that ship that way are the Docker-access proxy (whose own `SCHEMATIC.md` says it) and the router (whose `Containerfile` declares no `USER`) |
