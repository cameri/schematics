...
| `A-7` | the host container's process uid is not 0, and every other container of the set has its `id -u` read and reported | the host container's process runs as uid 0 | a uid cannot be read in a container (denied, absent): the row SKIPs naming it, because an unread container is not a clean one. R-4 forbids **host** root, so a uid of 0 inside another container that holds no host namespace, no socket and no bind of `/` is reported in a note rather than failed — naming the two parts that ship that way (the proxy, which states it in its own `SCHEMATIC.md`, and the router, whose `Containerfile` declares no `USER`) |
