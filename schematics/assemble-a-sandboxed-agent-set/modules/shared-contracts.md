...
| **The harness config root, and the files read under it** | `P-21 HARNESS_CONFIG_PATH` — the path(s) a deployment reads configuration from inside the container, **space-separated where the arm writes more than one** (the omp arm's endpoint and its roles live in two files, and every named path is read) | D-3's `P-9` (the same value, written into the image as `CLAUDE_CONFIG_DIR`/`CODEX_HOME`/`PI_CODING_AGENT_DIR`) | A deployment that reads or mounts a different directory sees an empty configuration while the CLI reads the real one; a deployment that names only one of two files has the other file's keys reported absent by `A-10` — a false failure on a valid deployment, not a wiring defect | `docker image inspect <harness image> --format '{{json .Config.Env}}'` against `P-21`, and every named file present in the image |
