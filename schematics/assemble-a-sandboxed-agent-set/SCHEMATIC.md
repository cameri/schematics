...
| P-21 | `HARNESS_CONFIG_PATH` | path (in-container), **space-separated when the arm writes more than one** | *(none — required)* | The layer part's harness-home parameter (`P-9`) plus the file name(s) that arm writes; the omp arm writes two — its endpoint and its roles | The CLI's configuration file(s) inside the image, **all of which `A-10` reads** to prove the wiring names only aliases and carries no provider credential. Naming one file where the arm writes two leaves the other file's keys reported absent, which reads as a wiring failure on a valid deployment |
