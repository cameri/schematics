...
| Age key unreadable (mode/ownership) | Decryption fails at boot — and **not** with a permission error: the decrypt tool skips a key file it cannot open and reports `Recovery failed because no master key was able to decrypt the file`, which reads like a wrong key *(measured 2026-09-19: key at mode 0600 owned by a different uid, exit 128)*. The key file must be readable by the container's user — uid 65532, the account R-12 fixes — and `D-3`'s `P-8` sets the file's mode from that same number |
