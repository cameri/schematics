...
#
# Prerequisites:
#   - authorize-docker-requests plugin installed and previously enabled
...
PLUGIN="authorize-docker-requests"
...
echo "==> Step 2: Bounce authorize-docker-requests plugin"
...
if docker plugin ls --format '{{.Name}}|{{.Enabled}}' 2>/dev/null | grep -q "^${PLUGIN}|true$"; then
    ...
    echo "    docker plugin install ghcr.io/open-policy-agent/authorize-docker-requests:v0.10" >&2
    exit 1
fi
echo ""
