...
- OPA plugin installation (the `authorize-docker-requests` managed plugin).
...
| D-3  | Authorize Docker Requests plugin  | Managed Docker plugin for Rego authz     | `docker plugin ls` for `authorize-docker-requests`                   | Hard fail in Phase 4 if install fails                     |
...
| P-10 | OPA_PLUGIN_TAG          | string | ghcr.io/open-policy-agent/authorize-docker-requests:v0.10 | `docker search authorize-docker-requests` for latest                               | Which OPA plugin image to install                                   |
...
The `authorize-docker-requests` plugin passes the Docker API request as this JSON
...
The plugin registers under the name `authorize-docker-requests` and is listed in
...
3. Append `"authorize-docker-requests"` to `authorization-plugins` list (deduplicate).
...
Verify:
```
python3 -c "import json; cfg = json.load(open('P-6/daemon.json')); assert cfg.get('tlsverify'), 'tlsverify missing'; assert 'authorize-docker-requests' in cfg.get('authorization-plugins', []), 'plugin missing'"
```

### Phase 4: Install OPA plugin
Goal: Install the authorize-docker-requests managed Docker plugin.

Steps:
1. Check if plugin exists: `docker plugin ls | grep authorize-docker-requests`.
2. If absent, install: `docker plugin install P-10`.
3. If present but disabled, enable: `docker plugin enable authorize-docker-requests`.

Verify:
```
docker plugin ls --format '{{.Name}} enabled={{.Enabled}}' | grep authorize-docker-requests | grep -q true
```
...
2. Bounce the plugin to compile the new policy:
   `docker plugin disable authorize-docker-requests && docker plugin enable authorize-docker-requests`
...
2. Test OPA denies unauthorized container creation:
   ...
   Should fail with "authorization denied by plugin authorize-docker-requests".
...
disable plugin (`docker plugin disable authorize-docker-requests`), fix policy, re-enable.
...
2. Disable OPA plugin:
   ```bash
   docker plugin disable authorize-docker-requests
   docker plugin rm authorize-docker-requests
   ```
...
- **Q-2**: The `resource_belongs_to_project` bind-mount path check hard-codes a
  ...
  `<P-3 project root>`. Should this be
...
- **Q-3**: The `authorize-docker-requests` managed plugin is at
  `ghcr.io/open-policy-agent/authorize-docker-requests:v0.10` which bundles OPA v0.60+.
