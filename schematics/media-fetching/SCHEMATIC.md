...
- Secrets via the sops-env-secrets pattern with per-service key names
...
- **R-6**: Credentials (VPN, Deluge web password, IP-sync cookie/state)
  MUST use the `sops-env-secrets` pattern: committed encrypted `.env`,
...
1. Deploy gluetun with the provider env (secrets via sops-env-secrets,
