...
|-----|------|------|------------|-----------|-------------------|
| D-1 | system | Docker Engine with BuildKit and Buildx (`docker buildx version` exits 0), with BuildKit actually usable by the build — a host where Buildx is installed but BuildKit is refused or disabled fails this dependency | Builds the image: `ARG TARGETARCH` is an automatic BuildKit build argument (the architecture guard reads it) and the install step uses `RUN --mount=type=cache`. Buildx then produces the manifest list (R-11) | `docker version`, `docker buildx version`, and the first build — the dependency is met only when a build gets past the architecture-validation step. A BuildKit-less build fails there with `ERROR: unsupported TARGETARCH ''` (measured on a legacy-builder host, exit 1) | Hard fail before Phase 2: without BuildKit there is no supported architecture-validated build, and without Buildx no multi-platform artifact |
...
| 2 | The build host lacks usable BuildKit (`DOCKER_BUILDKIT=0`, or a daemon that refuses BuildKit) | The build fails at the architecture validation step: `ERROR: unsupported TARGETARCH ''` (measured, exit 1). Passing `--build-arg TARGETARCH=amd64` clears that guard and the build then fails in the install step on `RUN --mount=type=cache`: `the --mount option requires BuildKit` (measured) | Build with BuildKit; there is no supported BuildKit-less build of the *base*. The architecture guard is deliberately there so an unvalidated build cannot pass silently, and the cache mounts have no legacy-builder equivalent. This is a dependency of the **base** only: a layer built from it adds nothing BuildKit-specific (`skeleton/Containerfile.layer.schema`) |
