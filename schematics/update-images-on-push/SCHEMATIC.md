...
- **R-5**: Docker API access MUST go through a deny-by-default scoped
  ...
  `restrict-docker-api-access` schematic implements this; this schematic treats
...
| D-5 | restrict-docker-api-access (see the restrict-docker-api-access schematic) | Scoped Docker API for the pull (R-5) | Its own deployment; this stack attaches to its network | Worker fails with 403; audit script diagnoses which group is missing |
...
| P-8 | SOCKET_PROXY_URL | string | `http://socket-proxy:2375` | The restrict-docker-api-access deployment | Where the worker POSTs pulls |
...
- **A-5** (R-5): the restrict-docker-api-access audit shows only the pull-only
...
- 2026-09-10: Reverse-engineered from a production deployment
  (a production VPS) that has run this exact pipeline since 2026-08-29. The
